...
const serverRender = async (context: ServerContext, opts = {}) => {
  ...
  if (process.env.NODE_ENV === 'development' && loadableStatsPath !== false) {
    ...
    loadableStatsPath = join(process.cwd(), 'build', 'loadable-stats.json');
  }
  const buildConfig = {
    loadableStatsPath,
    publicPath,
  ...
  const htmlTemplateContent =
    htmlTemplate || 'global.__ICE_SERVER_HTML_TEMPLATE__';
  ...
  const pageData = pagesData.find(
    ({ path, exact: end = false, strict = false, sensitive = false }) => {
      try {
        const regexp = (pathToRegexp.default || pathToRegexp)(path, [], {
          end,
          strict,
          sensitive,
        });
...
export async function renderStatic({
  ...
  try {
    ...
    if (getInitialProps) {
      console.log(
        '[SSR]',
        'getting initial props of page component',
        initialContext.pathname
      );
...
export function generateHtml({
  ...
  if (error) {
    $('head').append(`
      <script>
        window.__ICE_SSR_ERROR__ = "${(error instanceof Error
          ? error.message
          : error
        ).replace(/\"/g, "'")}";
      </script>`);
...
export async function getComponentByPath(routes, reqPath, i18nConfig) {
  ...
  async function findMatchRoute(routeList) {
    ...
    if (matchedRoute) {
      return matchedRoute.children
        ? await findMatchRoute(matchedRoute.children)
        : matchedRoute;
    }
  }
  ...
  return {
    ...
    getInitialProps:
      routeData?.getInitialProps ||
      routeData?.component?.getInitialProps ||
      loadedPageComponent?.default?.getInitialProps,
  };
}
