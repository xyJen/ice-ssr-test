...
function loopSplice(payload, collect, routerOptions) {
    ...
    sortedCollect.forEach(function (item) {
        ...
        if (!payload.nestImportsNames.includes(component)) {
            var nestImportsData = "import ".concat(component, " from '").concat(filePath, "';\n");
            if (lazy && !children) {
                nestImportsData = "const ".concat(component, " = lazy(() => import(/* webpackChunkName: '").concat(component, "' */ '").concat(filePath, "'));\n");
            }
        ...
        payload.nestSlice.push("\n".concat(indentTabs, "{\n  ").concat(indentTabs, "path: '").concat(routePathAmend, "',\n  ").concat(indentTabs, "exact: ").concat(isLayoutLike ? 'false' : defaultExact, ",\n  ").concat(indentTabs, "component: ").concat(component));
        if (children && children.length) {
            payload.nestSlice.push(",\n  ".concat(indentTabs, "children: "));
            // loop children
...
function splicer(routesCollect, routerOptions) {
    ...
    return "".concat(importAhead).concat(payload.nestImports.join(''), "\nexport default ").concat(payload.nestSlice.join(''), ";");
}
