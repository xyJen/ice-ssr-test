...
module.exports = function (config, hash, context) {
    ...
    if (hash) {
        ...
        var defaultHash = webpack.version.startsWith('4') ? 'hash:6' : 'fullhash:6';
        // can not use [chunkhash] or [contenthash] for chunk in dev mode
        var hashStr_1 = typeof hash === 'boolean' || command === 'start' ? defaultHash : hash;
        ...
        config.output.filename(formatPath(path.join(outputPath_1, "[name].[".concat(hashStr_1, "].js"))));
        if (config.plugins.get('MiniCssExtractPlugin')) {
            config.plugin('MiniCssExtractPlugin').tap(function (args) { return [
                Object.assign.apply(Object, __spreadArray(__spreadArray([], args, false), [{
                        filename: formatPath(path.join(outputPath_1, "[name].[".concat(hashStr_1, "].css"))),
                    }], false)),
            ]; });
        }
    }
