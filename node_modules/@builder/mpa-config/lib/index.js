...
var generateMPAEntries = function (api, options) {
    ...
    var _a = options.framework, framework = _a === void 0 ? 'rax' : _a, _b = options.targetDir, targetDir = _b === void 0 ? '' : _b, _c = options.executeGenerateTasks, executeGenerateTasks = _c === void 0 ? true : _c;
    ...
    entries.forEach(function (entry) {
        ...
        if (isAppEntry || (0, app_helpers_1.checkExportDefaultDeclarationExists)(path.join(rootDir, 'src', source))) {
            ...
            if (executeGenerateTasks) {
                result.generateTasks.forEach(function (generateTask) {
                    generateTask();
                });
            }
            else {
        ...
        parsedEntries[entryName] = __assign(__assign({}, entry), { generator: generator, generateTasks: generateTasks, finalEntry: finalEntry, runAppPath: runAppPath, routesFilePath: routesFilePath });
    });
...
var setMPAConfig = function (api, config, options) {
    ...
    Object.keys(parsedEntries).forEach(function (entryKey) {
        var _a = parsedEntries[entryKey], entryName = _a.entryName, finalEntry = _a.finalEntry, runAppPath = _a.runAppPath, routesFilePath = _a.routesFilePath;
    ...
    if (config.plugins.has('document')) {
        var filteredEntries_1 = __assign({}, parsedEntries);
        // remove property which is unnecessary for plugin document
        delete filteredEntries_1.generator;
        delete filteredEntries_1.generateTasks;
        config.plugin('document').tap(function (args) {
            return [__assign(__assign({}, args[0]), { pages: Object.values(filteredEntries_1) })];
        });
    }
    ...
    var matchInclude = function (filepath) {
        ...
        return matchReg && matchReg.test((0, app_helpers_1.formatPath)(filepath));
    };
