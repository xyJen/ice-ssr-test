                        ...
                        if (importMatchResult) {
                            ...
                            identifiers.forEach(function (identifier) {
                                ...
                                if (targetRedirect) {
                                    ...
                                    addImports_1.push("\n".concat(generateImport(targetRedirect)));
                                    importStr_1 = importStr_1.replace(new RegExp("".concat(identifier, ",?")), '');
                                }
                            });
                            newImportStr = '';
                            // Check should it retain original import str
                            if (addImports_1.length !== identifiers.length) {
                                newImportStr += "".concat(importStr_1, ";");
                            }
                            newImportStr += addImports_1.join(';');
                            return [2 /*return*/, "".concat(code.substring(0, targetImport.ss)).concat(newImportStr).concat(code.substring(targetImport.se))];
                        }
                            ...
                            return [2 /*return*/, "".concat(code.substring(0, targetImport.ss)).concat(generateImport(redirectImportInfo)).concat(code.substring(targetImport.se))];
                        }
                    }
...
function generateImport(redirectImportInfo) {
    var name = redirectImportInfo.name, redirectPath = redirectImportInfo.redirectPath, alias = redirectImportInfo.alias, exportDefault = redirectImportInfo.default;
    var importSpecifier = alias ? "".concat(alias, " as ").concat(name) : name;
    if (exportDefault) {
        return "import ".concat(importSpecifier, " from '").concat(redirectPath, "'");
    }
    return "import { ".concat(importSpecifier, " } from '").concat(redirectPath, "'");
}
