...
function matchAliasPath(_a) {
    var alias = _a.alias, value = _a.value, applyMethod = _a.applyMethod, rootDir = _a.rootDir;
    var srcRelativePath = '';
    Object.keys(alias).forEach(function (currKey) {
        if (value.startsWith(currKey)) {
            var _a = value.split(currKey), args = _a.slice(1);
            var absolutePagePath = applyMethod('formatPath', path.join.apply(path, __spreadArray([alias[currKey]], args, false)));
 ...
 */
function matchRelativePath(_a) {
    var routesPath = _a.routesPath, value = _a.value, applyMethod = _a.applyMethod, rootDir = _a.rootDir;
    var srcRelativePath = '';
    if (/^(\.\/|\.{2}\/)/.test(value)) {
        srcRelativePath = applyMethod('formatPath', path.relative(rootDir, path.join(routesPath, '..', value)));
    }
    return srcRelativePath;
}
/**
...
function formatPagePath(_a) {
    ...
    var matchedPagePath = matchRelativePath({ routesPath: routesPath, value: value, applyMethod: applyMethod, rootDir: rootDir }) || matchAliasPath({ alias: alias, value: value, applyMethod: applyMethod, rootDir: rootDir });
