...
var Generator = /** @class */ (function () {
    function Generator(_a) {
        ...
        this.render = function () {
            ...
            _this.renderData.runtimeModules = (0, getRuntimeModules_1.default)(_this.plugins, _this.targetDir, !!_this.renderData.hasJsxRuntime)
        ...
        this.addRenderFile = function (templatePath, targetPath, extraData) {
            ...
            if (renderIndex > -1) {
                ...
                if (targetTemplate[0] !== templatePath) {
                    _this.log.error('[template]', "path ".concat(targetPath, " already been rendered as file ").concat(targetTemplate[0]));
                }
            ...
            if (path.extname(templatePath) === '.ejs') {
                ...
                try {
                    ...
                    if (_this.showPrettierError) {
                        _this.log.warn("Prettier format error: ".concat(error.message));
    ...
    Generator.prototype.addContent = function (apiName) {
        ...
        if (!apiKeys.includes(apiName)) {
            throw new Error("invalid API ".concat(apiName));
        }
        var data = args[0], position = args[1];
        if (position && !['before', 'after'].includes(position)) {
            throw new Error("invalid position ".concat(position, ", use before|after"));
        }
        var registerKey = position ? "".concat(apiName, "_").concat(position) : apiName;
    ...
    Generator.prototype.generateImportStr = function (apiName) {
        ...
        return imports.map(function (_a) {
            ...
            return specifier ?
                "import ".concat(specifier, " from '").concat(source, "';") : "import '".concat(source, "'");
        }).join('\n');
    };
