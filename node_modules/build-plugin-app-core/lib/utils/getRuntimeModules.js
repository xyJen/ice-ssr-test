...
exports.default = (function (plugins, targetDir, hasJsxRuntime) {
    ...
    function analyzeRelativePath(filePath, relativeFiles) {
        ...
        imports.forEach(function (importName) {
            if (importName.startsWith('.')) {
                ...
                var basename = path.basename(importPath);
                // escape user folder with parentheses and spaces by glob file basename
                var importFile = globby.sync(path.extname(importPath) ? basename : ["".concat(basename, ".@((t|j)s?(x))"), "".concat(basename, "/index.@((t|j)s?(x))")], {
                ...
                if (importFile.length > 0) {
                    ...
                    throw new Error("can not find module ".concat(importName));
                }
            }
        });
    }
    return plugins.map(function (_a) {
        ...
        if (!absoluteModulePath) {
            ...
            if (fse.existsSync(srcDir)) {
                ...
                if (runtimePaths.length > 0) {
                    ...
                    modulePath = "../".concat(path.relative(targetDir, absoluteModulePath));
                }
            }
        }
        // read package.json
        ...
        try {
            ...
            console.log("ERROR: fail to load package.json of plugin ".concat(name));
        }
        return {
