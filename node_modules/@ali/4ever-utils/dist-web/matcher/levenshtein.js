...
export function levenshtein(a, b) {
  ...
  var dp = Array(2).fill(0).map(function () {
  ...
  for (var _i = 1; _i <= b.length; _i++) {
    for (var j = 0; j <= a.length; j++) {
      var row = _i % 2; // 初始位的距离为 b.substring(0, i) 的长度

      if (j === 0) {
        dp[row][j] = _i;
      } // 若字符相等，则距离为 lev(tail(a), tail(b))
      else if (a[j - 1] === b[_i - 1]) {
          dp[row][j] = dp[(_i - 1) % 2][j - 1];
        } // 若不相等，则距离为
        //           |--- lev(tail(a), b)
        // 1 + min --|--- lev(tail(a), tail(b))
        //           |--- lev(a, tail(b))
        else {
            dp[row][j] = 1 + Math.min(dp[(_i - 1) % 2][j], // delete b[i]
            dp[(_i - 1) % 2][j - 1], // replace a[j] with b[i]
