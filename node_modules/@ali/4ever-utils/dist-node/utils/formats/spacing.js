...
function linesToPx(lines, linePitch) {
  ...
  const pitch = linePitch || _theme.theme.spacing.defaultLinePitch; // px = lines * pitch
...
const ASL_LINE_HEIGHT = Object.keys(_theme.lineHeight.ACTUAL_MAP).map(h => parseFloat(h)).sort((a, b) => a - b);
...
function findClosestLineHeight(target) {
  let height = _theme.lineHeight.DEFAULT;
  let min = Infinity; // fast than some
...
function fromHeadingType(type) {
  ...
  return {
    // 转换为绝对值单位 px
    before: (0, _toUnits.ptToPx)(headStyle['margin-top']),
    after: (0, _toUnits.ptToPx)(headStyle['margin-bottom']),
    line: headStyle['line-height'] / _theme.lineHeight.DEFAULT
  };
}
/**
...
function fromHtml(value, fontSize) {
  ...
  const sz = _toUnits.unitsConverter.toPT(fontSize) || _theme.fontSize.DEFAULT;
  ...
  if (PER_REG.test(value)) {
    ...
    height = _toUnits.unitsConverter.toPT(value) / sz / _theme.lineHeight.DEFAULT;
  }
...
function fromBlock(paragraph, inheritedData, linePitch) {
  ...
  if (spacing.beforeAutospacing) {
    spacing.before = _theme.theme.spacing.defaultBeforeAutospacing; // 删除无效字段避免影响提取公共属性
    ...
    spacing.before = _toUnits.unitsConverter.toPX(spacing.before);
  }

  if (spacing.afterAutospacing) {
    spacing.after = _theme.theme.spacing.defaultAfterAutospacing;
    ...
    spacing.after = _toUnits.unitsConverter.toPX(spacing.after);
  }
  ...
  if ('line' in spacing) {
    const {
      line,
      lineRule = 'auto'
    } = spacing; // 存量数据中存在字符串类型的 line: 1.46667px（脏）,18px（正确）
    // 这里转换为倍数，同时防止 line 过小
    // word px 到 行高 公式：line = px / 16

    if (typeof line === 'string') {
      spacing.line = pxToTimes(_toUnits.unitsConverter.toPX(spacing.line)) || 1;
    }
...
function toStyle(block, inheritedData, linePitch) {
  const style = {}; // PERF: 提前 return 提升性能

  if (!block.data?.spacing && !inheritedData?.spacing && !_heading.headDefaultStyle[block.type]) {
    return style;
  }
  ...
  if (spacing.line) {
    ...
    if (lineRule === 'auto') {
      // 倍数
      style.lineHeight = `${_theme.lineHeight.ACTUAL_MAP[spacing.line] || spacing.line * _theme.lineHeight.DEFAULT}`;
...
function getLineHeightFromSpacing(spacing) {
  const defaultLineHeight = `${_theme.lineHeight.DEFAULT}`;
