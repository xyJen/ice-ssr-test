...
exports.getMarkTextMatchInfo = getMarkTextMatchInfo;
exports.getOffset = exports.getMatches = exports.re = void 0;
 ...
 * 获取字符的位置信息
 */
const getOffset = (snippetEnd, snippet, borderSize) => ({
 ...
 */
function getMarkTextMatchInfo(text, delimiter, insideWhitespace = false) {
  const {
    character,
    size,
    disableIntraword
  } = delimiter; // 排除为空白字符串、仅包含 delimiter 的情况
  ...
  const normalMatches = text.match(RegExp( // 例如 bold: \*{2}      (     [^\*\s]+       )          \*{2} $
  ...
  if (!insideWhitespace) {
    ...
    const whiteSpaceMatches = text.match(RegExp( // 例如 bold: \*{2}      (     [^\*\s]+             [^\*]*            [^\*\s]+        )    \*{2} $
