...
exports.default = updateDragHighlight;
...
function updateDragHighlight(controller, payload) {
  const {
    key,
  ...
  if (block?.data?.calloutPr || config && config.enableSubjectGroup && Heading1.isHeading1(block) && block.data.subjectPr) {
    ...
    if (group) {
      if (isTableHighlight) {
        (0, _domUtils.addDragTableHighlight)(group);
      } else {
        (0, _domUtils.addDragHighlight)(group);
      }
  ...
  blocks?.forEach(node => {
    ...
    if (isTableHighlight) {
      (0, _domUtils.addDragTableHighlight)(dom);
    } else {
      (0, _domUtils.addDragHighlight)(dom, enableBorderHighlight);
    }
  });
