...
describe('Bi/draggable/thumbnails/mix', () => {
  afterEach(() => {
    (0, _react2.cleanup)();
    jest.clearAllMocks();
  });
  test('mix blocks thumbnail', async () => {
    const value = _ref2;
    const {
      container,
      controller
    ...
    await (0, _react2.waitFor)(() => {
      expect(container.querySelectorAll(`.${_everPluginDraggable.DRAG_HIGHLIGHT_CLASSNAME}`).length).toBe(3);
    });
    const thumbnail = getDragThumbnails(controller, {
      viewKey: 'a'
    });
    expect(thumbnail.firstElementChild.childElementCount).toBe(3);
    expect(!!thumbnail.querySelector('[data-cangjie-key="a"]')).toBe(true);
    expect(!!thumbnail.querySelector('[data-cangjie-key="b"]')).toBe(false);
    expect(!!thumbnail.querySelector('[data-cangjie-key="c"]')).toBe(true);
  });
});
//# sourceMappingURL=thumbnails.mix.ui.test.js.map