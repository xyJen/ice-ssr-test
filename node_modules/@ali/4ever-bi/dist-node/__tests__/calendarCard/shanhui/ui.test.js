...
var _types = require("../../../plugins/calendarCard/types");
...
const createCalendarConfig = config => {
  return {
    mode: _types.CalendarType.SHANHUI,
    locale: {
      refresh: '刷新',
      refreshTips: '日程卡片有更新',
...
describe('Bi/card/ui', () => {
  ...
  describe('Bi/CalendarCard/UI', () => {
    ...
    test('点击交互', async () => {
      ...
      expect(!handleState.refresh.eventType).toBe(true); // 点击goto
    ...
    test('已有日程数据，点击刷新 icon 写入数据', async () => {
      ...
      expect(!!isUpdate).toBe(false); // 点击刷新按钮
