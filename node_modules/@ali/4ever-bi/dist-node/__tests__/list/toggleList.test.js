...
describe('list', () => {
    ...
    [{
      ...
      for (const enumKey of Object.keys(defaultListArray)) {
        ...
        it(`选中有序列表类型1 切换为 ${name + (index + 1)}`, async () => {
          ...
          clickToolbar(container, `[data-testid="bi-toolbar-${type}list"] .wdn-toolbar-split-btn-arrow`);
  ...
  describe('选中列表内容切换为标题或正文, 项目符号继承标题或正文样式', () => {
    [{
      ...
      var _ref22 = (0, _everDevTest.jsx)(_editor.default, {
        defaultValue: value
      });
      ...
      for (const textType of HEADING_SZ) {
        it(`选中${selectStatus}列表内容,通过toolbar设置列表内容为${textType}类型，`, async () => {
          ...
          await (0, _react.waitFor)(() => queryByTestId('bi-toolbar-heading'));
  ...
  describe('多行列表，每一行设置不同的段落格式，取消多行列表的列表格式后，各行仍保持原有独立的段落格式', () => {
    ...
    var _ref26 = (0, _everDevTest.jsx)(_editor.default, {
      defaultValue: value
    });
    ...
    it('多行列表', async () => {
      ...
      const btn = await (0, _react.waitFor)(() => queryByTestId('bi-toolbar-align__list_center'));
      ...
      await (0, _react.waitFor)(() => queryByTestId('bi-toolbar-ullist'));
      clickToolbar(container, '[data-testid="bi-toolbar-ullist"] .wdn-toolbar-split-left');
...
//# sourceMappingURL=toggleList.test.js.map