...
var _propertyUtils = require("./property-utils");
...
function setLeftTop(elem, offset, option) {
  ...
  if (_utils.utils.css(elem, 'position') === 'static') {
  ...
  // 复原回原来的样式值，取消掉原来预设的值
  _utils.utils.css(elem, originalStyle);
  ...
  for (const key in offset) {
    if (Object.prototype.hasOwnProperty.call(offset, key)) {
      ...
      if (key === dir) {
        ret[dir] = parseFloat(originalStyle[dir]) + off;
      } else {
        ret[dir] = parseFloat(originalStyle[dir]) - off;
      }
    }
  }
  _utils.utils.css(elem, ret);
}
function setTransform(elem, offset) {
  const originalOffset = getOffset(elem);
  const originalXY = (0, _propertyUtils.getTransformXY)(elem);
  const resultXY = {
    x: originalXY.x,
    y: originalXY.y
  };
...
function setOffset(elem, offset, option) {
  if (option.ignoreShake) {
    ...
    const tLeft = (_offset$left = offset.left) === null || _offset$left === void 0 ? void 0 : _offset$left.toFixed(0);
    const tTop = (_offset$top = offset.top) === null || _offset$top === void 0 ? void 0 : _offset$top.toFixed(0);
