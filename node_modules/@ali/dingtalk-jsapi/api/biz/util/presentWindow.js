"use strict";
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.presentWindow$ = void 0;
var ddSdk_1 = require("../../../lib/ddSdk");
var apiName = 'biz.util.presentWindow';
ddSdk_1.ddSdk.setAPI(apiName, (_a = {},
    _a[ddSdk_1.ENV_ENUM.ios] = { vs: '2.8.0' },
    _a[ddSdk_1.ENV_ENUM.android] = { vs: '2.8.0' },
    _a));
/**
 * 打开窗口
 * @apiName biz.util.presentWindow
 * @supportVersion  ios: 2.8.0 android: 2.8.0
 */
function presentWindow$(params) {
    return ddSdk_1.ddSdk.invokeAPI(apiName, params);
}
exports.presentWindow$ = presentWindow$;
exports.default = presentWindow$;
