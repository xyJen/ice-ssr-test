"use strict";
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.openPrintWnd$ = void 0;
var ddSdk_1 = require("../../../lib/ddSdk");
var apiName = 'biz.util.openPrintWnd';
ddSdk_1.ddSdk.setAPI(apiName, (_a = {},
    _a[ddSdk_1.ENV_ENUM.pc] = { vs: '6.0.8' },
    _a));
/**
 * 打开钉盘文件打印窗口
 * @apiName biz.util.openPrintWnd
 * @supportVersion win: 6.0.8 mac: 6.0.8
 * @author win:周镛 mac:伯温
 */
function openPrintWnd$(params) {
    return ddSdk_1.ddSdk.invokeAPI(apiName, params);
}
exports.openPrintWnd$ = openPrintWnd$;
exports.default = openPrintWnd$;
