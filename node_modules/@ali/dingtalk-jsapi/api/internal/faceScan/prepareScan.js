"use strict";
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.prepareScan$ = void 0;
var ddSdk_1 = require("../../../lib/ddSdk");
var apiName = 'internal.faceScan.prepareScan';
ddSdk_1.ddSdk.setAPI(apiName, (_a = {},
    _a[ddSdk_1.ENV_ENUM.ios] = { vs: '3.4.6' },
    _a[ddSdk_1.ENV_ENUM.android] = { vs: '3.4.6' },
    _a));
/**
 * 人脸识别准备，成功后再调用scan
 * @apiName internal.faceScan.prepareScan
 * @supportVersion  ios: 3.4.6 android: 3.4.6
 */
function prepareScan$(params) {
    return ddSdk_1.ddSdk.invokeAPI(apiName, params);
}
exports.prepareScan$ = prepareScan$;
exports.default = prepareScan$;
