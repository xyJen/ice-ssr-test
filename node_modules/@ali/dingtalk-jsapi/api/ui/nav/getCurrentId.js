"use strict";
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.getCurrentId$ = void 0;
var ddSdk_1 = require("../../../lib/ddSdk");
var apiName = 'ui.nav.getCurrentId';
ddSdk_1.ddSdk.setAPI(apiName, (_a = {},
    _a[ddSdk_1.ENV_ENUM.ios] = { vs: '2.6.0' },
    _a[ddSdk_1.ENV_ENUM.android] = { vs: '2.6.0' },
    _a));
/**
 *
 * @apiName ui.nav.getCurrentId
 * @supportVersion  ios: 2.6.0 android: 2.6.0
 */
function getCurrentId$(params) {
    return ddSdk_1.ddSdk.invokeAPI(apiName, params);
}
exports.getCurrentId$ = getCurrentId$;
exports.default = getCurrentId$;
