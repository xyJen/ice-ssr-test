...
import { EditorPreview, EditorMdObj } from './previewOnly';
import { EditorInput, use } from './editorOnly';
import MdChangeTypePlugin from './plugins/richText'; // 注册默认插件

Editor.use(Header);
Editor.use(FontBold);
Editor.use(FontItalic); // Editor.use(FontUnderline);

Editor.use(FontStrikethrough);
Editor.use(ListUnordered);
Editor.use(ListOrdered);
Editor.use(BlockQuote); // Editor.use(BlockWrap);
// Editor.use(BlockCodeInline);

Editor.use(BlockCodeBlock);
Editor.use(Table);
Editor.use(Image);
Editor.use(Link); // Editor.use(Clear);
// Editor.use(Logger);

Editor.use(ModeToggle); // Editor.use(FullScreen);

Editor.use(TabInsert);
use(MdChangeTypePlugin);
use(Header);
use(FontBold);
...
use(Link);
use(ListUnordered);
use(ListOrdered); // use(FullScreen);
// use(BlockWrap);
// use(BlockCodeInline);

use(Image);
use(ModeToggle); // use(Clear);

use(Logger);
use(TabInsert); // 导出声明
// 导出工具组件

export { EditorInput, use as usePlugin };
export { EditorToolbar };
var EditorMd = EditorMdObj;
export { EditorPreview, EditorMd };
export { DropList };
