...
var _emojiApi = _interopRequireDefault(require("../plugins/emoji/emojiApi"));
...
for (var i = 1; i <= 6; i++) {
  SIMPLE_DECORATOR["h" + i] = [(0, _tool.repeat)('#', i) + " ", ''];
} // 插入emoji


Object.keys(_emojiApi.default).forEach(function (key) {
  SIMPLE_DECORATOR["" + key] = [":" + key + ":", ''];
});
...
function getDecorated(target, type, option) {
  ...
  switch (type) {
    ...
    case 'shiftTab':
      var removeValue = option.tabMapValue === 1 ? '\t' : ' '.repeat(option.tabMapValue);
      var newRemovedText = target.split('\n').map(function (item) {
        return item.replace(removeValue, '');
      }).join('\n');
      var lineBreakCountRemove = target.includes('\n') ? target.match(/\n/g).length : 0;
      return {
        text: newRemovedText,
        selection: {
          start: 0,
          end: option.tabMapValue * (lineBreakCountRemove + 1) + target.length
        }
      };
