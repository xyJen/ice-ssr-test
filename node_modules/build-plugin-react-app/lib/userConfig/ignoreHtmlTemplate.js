module.exports = function (config, ignoreHtmlTemplate, context) {
    var command = context.command, userConfig = context.userConfig;
    if (command === 'build' && ignoreHtmlTemplate) {
        var entry = userConfig.entry;
        if (Object.prototype.toString.call(entry) === '[object Object]' && Object.keys(entry).length > 1) {
            // delete multi HtmlWebpackPlugin
            Object.keys(entry).forEach(function (entryKey) {
                config.plugins.delete("HtmlWebpackPlugin_".concat(entryKey));
            });
        }
        else {
            config.plugins.delete('HtmlWebpackPlugin');
        }
    }
};
//# sourceMappingURL=ignoreHtmlTemplate.js.map